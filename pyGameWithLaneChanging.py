...
# Alright y'all read here for the dealio,......................................
# This script will launch a pygame window with a randomly spawned vehicle 
# in the world that is running on the server.
# when the window is opened cruise control will be on automatically. 
# to turn it off hit the spacebar.
# to turn it back on hit the 'w' or up key. 
# thats pretty much it; notice how this was just the 'manual_control.py' file 
# that I edited as necesssary.
# Below you will find further comments of the changes I have made.
# variable usage: line 162, 227, 237, 302, 303 
# something important: line 445 
# implementing cruise control: line 466
#  
"""
...
class KeyboardControl(object):
    ...
    def parse_events(self, client, world, clock):
        ...
        if not self._autopilot_enabled:
            if isinstance(self._control, carla.VehicleControl):
                ...
                #############HUGE################################
                # okay this is not that huge, but this is where pygame is listening for keys to be pressed
                # and its important to call this function with 'world.player' passed in 
    ...
    def _parse_vehicle_keys(self, keys, milliseconds, vehicle, client, world):
        ...
        if self.cruiseControl == True: #if statement to run cruise control 
            #sets the returned throttle value from our PID to the actual throttle 
            self._control.throttle = longPID(vehicle) 
            #print('cruise control is on!!')
        else:
            self._control.throttle = 0.0 #if not then just no throttle
        
        #if statement to turn CC off
        if keys[K_SPACE]: #if space bar is pressed
            self.cruiseControl = False #set boolean to false
            #print('cruise control is now off :( :( :(')
        ...
        if self.lateralControl == True: #if statement to implement autonomous lateral control
            ...
            self._control.steer, self.leftLaneChange, self.rightLaneChange, self.x_actualPath, self.y_actualPath = latPID(vehicle, wrld, self.leftLaneChange, self.rightLaneChange, self.x_actualPath, self.y_actualPath) 
            ...
            for d, vehicle in sorted(vehicles):
                ...
                if d < 15 and d > 8 and self.hi == 3:
                    ...
                    self.rightLaneChange = False   
                elif d < 4 and self.hi == 3: 
                    self.hi = 4
                elif d > 20 and d < 21 and self.hi == 4:
                    self.hi = 7
                    
                if d < 15 and d > 8 and self.hi == 7:
                    self.leftLaneChange = True
                    
                    print(self.hi)
                    print('lane change initiated!!')
                    print(self.leftLaneChange)
                elif d < 6 and d > 4:
                    self.leftLaneChange = False   
                elif d < 4 and self.hi == 7:
                    self.hi = 5
                elif d > 20 and d < 21 and self.hi == 5:
                    self.hi = 3
                ##
